...
class Configuration:
  ...
  def write_combined_migration(self,  f ):
      ...
      # write for servers
      # populate the serveraddr array
      #print "num_servers %d" %num_servers
      #for k in self.ipaddr:
      #  print k
      for j in range(num_servers):
          #sid = (1 + j % self.num_server_machines) % self.num_machines
          sid = j
          serveraddr.append("%s:%s" % ( self.ipaddr[sid], options.port+sid*self.port_shift) )
          #i += 1


      initial_no_migration_days = int(param["INITIAL_NO_MIGRATION_DAYS"])
      final_no_migration_days = int(param["FINAL_NO_MIGRATION_DAYS"])
      assert initial_no_migration_days >= 0 and initial_no_migration_days <= self.num_days
      assert final_no_migration_days >= 0 and final_no_migration_days <= self.num_days
      migration_days = self.num_days - initial_no_migration_days - final_no_migration_days
      ...
      for d in range(initial_no_migration_days, initial_no_migration_days + migration_days):
          ...
          sid = (1 + i % self.num_server_machines) % self.num_machines
